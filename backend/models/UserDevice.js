...
async function upsertDevice({ userId, fcmToken, deviceId, deviceName, manufacturer, model, platform, osVersion, appVersion }) {
  ...
  const query = `
    INSERT INTO user_devices (
      ...
      platform, os_version, app_version, app_status, notification_status, uninstall_detected_at, last_seen_at, updated_at
    ) 
    VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, 'active', 'active', NULL, NOW(), NOW())
    ...
    RETURNING *;
  `;
  const values = [userId, fcmToken, deviceId, deviceName, manufacturer, model, platform, osVersion, appVersion];
...
async function heartbeat(userId, deviceId, appVersion, osVersion) {
  const query = `
    ...
    RETURNING *;
  `;
  const result = await pool.query(query, [appVersion, osVersion, userId, deviceId]);
  return result.rows[0];
}
